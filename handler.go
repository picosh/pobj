...
func (h *UploadAssetHandler) Read(s ssh.Session, entry *utils.FileEntry) (os.FileInfo, utils.ReaderAtCloser, error) {
	...
	fname := h.Cfg.AssetNames.FileName(entry)
	contents, size, modTime, err := h.Cfg.Storage.GetFile(bucket, fname)
	if err != nil {
		return nil, nil, err
	}
...
func (h *UploadAssetHandler) List(s ssh.Session, fpath string, isDir bool, recursive bool) ([]os.FileInfo, error) {
	...
	if cleanFilename == "" || cleanFilename == "." {
		...
		foundList, err := h.Cfg.Storage.ListFiles(bucket, cleanFilename, recursive)
		if err != nil {
			return fileList, err
		}
